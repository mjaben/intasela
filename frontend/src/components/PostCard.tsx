...
export default function PostCard({ 
  ...
  const handleAction = (e: React.MouseEvent) => {
    ...
    // Else perform action (e.g., like, comment, resela)
  };

  return (
    <article className="border-b border-border p-6 hover:bg-accent/30 transition-colors cursor-pointer" onClick={() => {/* Navigate to post detail */}}>
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Body */}
          <div className="text-[15px] leading-relaxed mb-4 text-foreground/90 break-words prose prose-invert max-w-none">
            ...
            {resela && (
              <div className="mt-4 border border-border rounded-xl p-4 bg-background hover:bg-accent/30 transition-colors">
                ...
                <div className="text-[14px] text-foreground/90 prose prose-invert max-w-none">
                  <ReactMarkdown>{resela.content}</ReactMarkdown>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between text-muted-foreground text-sm max-w-md">
            ...
            {/* 1. Heart (Like) */}
            <button onClick={handleAction} className="flex items-center gap-3 hover:text-red-500 transition-colors group">
            ...
            {/* 2. Comment */}
            <button onClick={handleAction} className="flex items-center gap-3 hover:text-blue-500 transition-colors group">
            ...
            {/* 3. Resela */}
            <button onClick={handleAction} className="flex items-center gap-3 hover:text-green-500 transition-colors group" title="Resela (Repost)">
            ...
            {/* 4. Impressions */}
            <button onClick={handleAction} className="flex items-center gap-3 hover:text-primary transition-colors group" title="Post Impressions">
            ...
            {/* 5. Share */}
            <button onClick={handleAction} className="p-2 rounded-full hover:bg-accent hover:text-foreground transition-colors group" title="Share">
