...
export default function SpaceCard({ id, name, description, coverUrl, type, membersCount, isMember, sampleMembers = [] }: SpaceCardProps) {
  return (
    <Link href={`/spaces/${id}`} className="block border border-border rounded-xl overflow-hidden hover:border-[#ACC8A2] transition-colors bg-card/50 backdrop-blur-sm group">
      ...
      <div className="p-4">
        <h3 className="text-lg font-bold truncate group-hover:text-[#ACC8A2] transition-colors">{name}</h3>
        ...
        <div className="mt-4 flex items-center justify-between text-sm">
          ...
          {isMember && <span className="text-[#ACC8A2] bg-[#ACC8A2]/10 px-2 py-1 rounded font-bold text-xs">Joined</span>}
        </div>
      </div>
    </Link>
  );
