...
function OrbitContent() {
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    // Swipe left (next tab)
    if (diff > 50 && feedType === 'following') {
      setFeedType('for_you');
    }
    // Swipe right (prev tab)
    else if (diff < -50 && feedType === 'for_you') {
  ...
  return (
    <div className="w-full h-[calc(100dvh-60px)] md:h-[100vh] -mb-32 md:-mb-0 bg-[#0f150e] relative">
...
function OrbitPlayer({ post }: { post: any }) {
  ...
  const isWideVideo = aspectRatio !== null && aspectRatio > 0.65;
  ...
  const handleInterested = async (e: React.MouseEvent) => {
    ...
    if (!isAuthenticated) return router.push("/login");
    setShowOptionsMenu(false);
    addToast("We'll show more videos like this", "success");
  ...
  const handleNotInterested = async (e: React.MouseEvent) => {
    ...
    if (!isAuthenticated) return router.push("/login");
    setShowOptionsMenu(false);
    addToast("We'll show fewer videos like this", "success");

    // Skip to next video
    const nextVideo = containerRef.current?.nextElementSibling;
    if (nextVideo) {
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-[calc(100dvh-60px)] md:h-[100vh] md:py-6 snap-start snap-always flex items-center justify-center bg-transparent relative"
    >
      {/* Facebook/TikTok Style Container */}
      <div className="relative flex flex-col w-full h-full md:w-auto md:max-w-full md:rounded-[20px]">
        {/* Video Box */}
        <div className={`w-full relative bg-[#0a0a0a] md:rounded-[20px] overflow-hidden flex items-center justify-center shadow-2xl shrink-0 ${isWideVideo ? 'h-auto max-h-[55vh]' : 'h-full'}`}>
          {/* Blurred Background for wide videos */}
          <div 
            className="absolute inset-0 w-full h-full bg-center bg-cover blur-2xl opacity-90 scale-125 pointer-events-none"
          ...
          <video
            ...
            className={`w-full cursor-pointer relative z-10 ${isWideVideo ? 'object-contain h-auto max-h-[55vh]' : 'object-cover h-full'}`}
            loop
            playsInline
            onClick={togglePlay}
          />
          ...
          {!isWideVideo && (
            <div 
              className="absolute bottom-0 left-0 w-full p-4 md:pb-6 pt-16 bg-gradient-to-t from-black/80 via-black/40 to-transparent pointer-events-none z-20"
            ...
            <div className="flex flex-col gap-2 max-w-[85%] md:max-w-full pointer-events-auto">
              ...
              <div className="pointer-events-auto">
                <div className={`overflow-y-auto no-scrollbar transition-all ${isCaptionExpanded ? 'max-h-[35vh]' : ''}`}>
                  <p className={`text-white/90 text-[15px] leading-snug drop-shadow-md ${isCaptionExpanded ? '' : 'line-clamp-2'}`}>
                    {post.content}
                  </p>
                </div>
                {post.content && post.content.length > 70 && (
                  <button 
                    ...
                    className="text-white/80 text-[13px] font-bold mt-1 hover:text-white"
                  >
        ...
        {!isWideVideo && (
          <div 
            className="absolute right-2 md:-right-[60px] flex flex-col gap-3 items-center z-30 md:!bottom-4"
          ...
          <button onClick={handleResela} className="flex flex-col items-center gap-1 group">
            <div className="w-10 h-10 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center transition group-hover:scale-110">
              <div
                className={`w-[20px] h-[20px] transition-colors ${isReselaed ? 'bg-[#ACC8A2]' : 'bg-white'}`}
                style={{
          ...
          <button onClick={handleBookmark} className="flex flex-col items-center gap-1 group">
            <div className="w-10 h-10 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center transition group-hover:scale-110">
              <div
                className={`w-[20px] h-[20px] transition-colors ${isBookmarked ? 'bg-[#ACC8A2]' : 'bg-white'}`}
                style={{
              ...
              <>
                ...
                <div className="absolute bottom-full right-full mb-2 mr-2 z-20 bg-[#18181b]/90 backdrop-blur-md border border-gray-700/50 rounded-xl shadow-2xl p-2 flex flex-col min-w-[220px]">
                  <button
                    ...
                    className="w-full px-2.5 py-2 hover:bg-white/10 text-left rounded-lg text-white/90 hover:text-white font-medium flex items-center gap-2.5 transition-colors text-[13px]"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-2.5 py-2 hover:bg-white/10 text-left rounded-lg text-white/90 hover:text-white font-medium flex items-center gap-2.5 transition-colors text-[13px] mt-0.5"
                  >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3zm7-13h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17"></path></svg>
                    Not interested
                  </button>
        ...
        {/* Split Layout Content (For Wide Videos) */}
        {isWideVideo && (
          <div className="flex-1 w-full bg-transparent flex flex-col p-4 pb-24 md:pb-4 overflow-y-auto relative z-20">
            {/* Author Info */}
            <div className="flex items-center gap-2 mb-3">
              <div className="flex items-center gap-1.5 cursor-pointer" onClick={() => router.push(`/@${post.author.username}`)}>
                <div className="w-9 h-9 rounded-full overflow-hidden border border-white/20 bg-black shadow-md shrink-0">
                  <img src={post.author.avatarUrl || `https://api.dicebear.com/7.x/notionists/svg?seed=${post.author.username}`} alt="Avatar" className="w-full h-full object-cover" />
                </div>
                <div className="flex flex-col">
                  <span className="text-white font-bold text-[14px] hover:underline leading-tight">@{post.author.username}</span>
                  <span className="text-white/50 text-[12px] leading-tight">{post.author.name}</span>
                </div>
              </div>
              {currentUser?.username !== post.author.username && !isFollowing && (
                <button
                  ...
                  className="ml-auto text-primary font-bold text-[12px] px-4 py-1.5 rounded-full border border-primary/40 hover:bg-primary/20 bg-black/40 transition-colors uppercase tracking-wide shadow-md"
                >
                  {post.author.isFollower ? "Follow Back" : "Follow"}
                </button>
              )}
            ...
            {/* Caption */}
            <div className="mb-4">
              <p className={`text-white/90 text-[14px] leading-relaxed ${isCaptionExpanded ? '' : 'line-clamp-2'}`}>
                {post.content}
              </p>
              {post.content && post.content.length > 70 && (
                <button 
                  onClick={(e) => { e.stopPropagation(); setIsCaptionExpanded(!isCaptionExpanded); }}
                  className="text-white/60 text-[13px] font-bold mt-1 hover:text-white"
                >
                  {isCaptionExpanded ? "Show less" : "See more..."}
                </button>
              )}
            ...
            <div className="flex items-center justify-between border-y border-white/10 py-3 mb-5">
              <button onClick={handleLike} className="flex items-center gap-1.5 group">
                <div
                  className={`w-[22px] h-[22px] transition-colors ${isLiked ? 'bg-red-500' : 'bg-white'}`}
                  style={{
                    WebkitMaskImage: 'url(data:image/png;base64,)',
                    WebkitMaskSize: 'contain',
                    WebkitMaskRepeat: 'no-repeat',
                ...
                />
                <span className="text-white/90 text-[13px] font-bold">{likeCount}</span>
              </button>
              
              <button onClick={handleResela} className="flex items-center gap-1.5 group">
                <div
                  className={`w-[22px] h-[22px] transition-colors ${isReselaed ? 'bg-[#ACC8A2]' : 'bg-white'}`}
                  style={{
                    WebkitMaskImage: 'url(data:image/png;base64,)',
                    WebkitMaskSize: 'contain',
                    WebkitMaskRepeat: 'no-repeat',
                ...
                />
                <span className="text-white/90 text-[13px] font-bold">{reselaCount}</span>
              </button>

              <button className="flex items-center gap-1.5 group" onClick={() => router.push(`/@${post.author.username}/posts/${post.id}`)}>
                <div
                  className="w-[22px] h-[22px] bg-white transition-opacity opacity-90 group-hover:opacity-100"
                  style={{
                    WebkitMaskImage: 'url(data:image/png;base64,)',
                    WebkitMaskSize: 'contain',
                    WebkitMaskRepeat: 'no-repeat',
                ...
                />
                <span className="text-white/90 text-[13px] font-bold">{post.stats?.replies || 0}</span>
              </button>

              <button onClick={handleBookmark} className="flex items-center gap-1.5 group">
                <div
                  className={`w-[22px] h-[22px] transition-colors ${isBookmarked ? 'bg-[#ACC8A2]' : 'bg-white'}`}
                  style={{
                    WebkitMaskImage: 'url(data:image/png;base64,)',
                    WebkitMaskSize: 'contain',
                    WebkitMaskRepeat: 'no-repeat',
              ...
              <button className="flex items-center gap-1.5 group" onClick={handleShareMenuClick}>
                <div
                  className="w-[22px] h-[22px] bg-white transition-opacity opacity-90 group-hover:opacity-100"
                  style={{
                    WebkitMaskImage: 'url(data:image/png;base64,)',
            ...
            {/* Quick Comment Bar */}
            <div 
              onClick={() => router.push(`/@${post.author.username}/posts/${post.id}`)}
              className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-full px-4 py-2.5 mb-6 cursor-text"
            >
              <div className="w-6 h-6 rounded-full overflow-hidden bg-black shrink-0">
                <img src={currentUser?.avatarUrl || `https://api.dicebear.com/7.x/notionists/svg?seed=${currentUser?.username}`} className="w-full h-full object-cover" />
              </div>
              <span className="text-white/40 text-[14px]">Add a comment...</span>
            </div>
...
export default function OrbitFeed() {
  return (
    <Suspense fallback={
      <div className="w-full h-screen bg-[#0f150e] flex items-center justify-center">
