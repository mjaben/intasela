            ...
            {showShareMenu && (
              <>
                ...
                <div className="absolute bottom-full right-full mb-2 mr-2 z-20 bg-[#18181b]/90 backdrop-blur-md border border-gray-700/50 rounded-xl shadow-2xl p-2 flex flex-col min-w-[220px]">
                  ...
                  <button 
                    onClick={(e) => { e.stopPropagation(); window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(`Check out this post by ${post.author.name}`)}&url=${encodeURIComponent(`${window.location.origin}/@${post.author.username}/posts/${post.id}`)}`, '_blank'); setShowShareMenu(false); }} 
                    className="p-3 hover:bg-gray-800/80 text-left rounded-lg text-white font-medium flex items-center gap-3 transition-colors text-[13px]"
                  >
                  ...
                  <button 
                    onClick={(e) => { e.stopPropagation(); window.open(`https://api.whatsapp.com/send?text=${encodeURIComponent(`Check out this post by ${post.author.name}: ${window.location.origin}/@${post.author.username}/posts/${post.id}`)}`, '_blank'); setShowShareMenu(false); }} 
                    className="p-3 hover:bg-gray-800/80 text-left rounded-lg text-white font-medium flex items-center gap-3 transition-colors text-[13px]"
                  >
